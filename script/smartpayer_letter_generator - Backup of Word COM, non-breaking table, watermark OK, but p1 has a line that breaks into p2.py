...
def _set_table_page_margins(doc):
    ...
    if pgMar is None:
        ...
        sectPr.append(pgMar)
    # top/bottom/header/footer match the template so Word COM lays out
    # the inherited header1.xml / footer1.xml content without needing
    # an extra continuation page.  left/right stay narrow (1 cm each)
    # to give the table ~19 cm usable width.
    pgMar.set(qn("w:top"),    "1440")  # 2.54 cm — matches template
    pgMar.set(qn("w:bottom"), "1440")  # 2.54 cm — matches template
    pgMar.set(qn("w:left"),   "567")   # 1.00 cm — narrow for table
    pgMar.set(qn("w:right"),  "567")   # 1.00 cm — narrow for table
