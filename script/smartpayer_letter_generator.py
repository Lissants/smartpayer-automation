...
def _set_table_page_margins(doc):
    ...
    if pgMar is None:
        ...
        sectPr.append(pgMar)
    # Standard margins that match header/footer content size
    pgMar.set(qn("w:top"),    "1440")  # 2.54 cm — standard
    pgMar.set(qn("w:bottom"), "1440")  # 2.54 cm
    pgMar.set(qn("w:left"),   "567")   # 1.00 cm — narrow for table width
    pgMar.set(qn("w:right"),  "567")   # 1.00 cm
    pgMar.set(qn("w:header"), "2268")  # 4.00 cm — matches header1.xml
    pgMar.set(qn("w:footer"), "2268")  # 4.00 cm — matches footer1.xml
...
def _split_sections_before_table(doc):
    """
    Create a CONTINUOUS section break before the table so:
    - Letter section keeps compact margins (content fits on page 1)
    - Table section uses standard margins (matches header/footer size)
    - NO extra blank pages are inserted
    """
    import copy
    from docx.oxml import OxmlElement
    from docx.oxml.ns import qn
    ...
    if body_sectPr is None:
        return  # _set_table_page_margins will create one from scratch

    # Clone → becomes the letter section's settings (compact margins)
    letter_sectPr = copy.deepcopy(body_sectPr)

    # Compact top/bottom on letter pages so filled content fits on ONE page.
    # 1080 twips = 1.9 cm — enough room for real variable values without overflow
    pgMar = letter_sectPr.find(qn("w:pgMar"))
    if pgMar is None:
        pgMar = OxmlElement("w:pgMar")
        letter_sectPr.append(pgMar)
    pgMar.set(qn("w:top"),    "1080")   # ≈ 1.9 cm — content fits without overflow
    pgMar.set(qn("w:bottom"), "1080")   # ≈ 1.9 cm
    # left/right/header/footer keep template defaults

    # Attach the letter sectPr to the last existing paragraph as a
    # CONTINUOUS section break (no page break).
    if doc.paragraphs:
        carrier = doc.paragraphs[-1]._p
    else:
        carrier = OxmlElement("w:p")
        body.insert(list(body).index(body_sectPr), carrier)

    pPr = carrier.find(qn("w:pPr"))
    if pPr is None:
        pPr = OxmlElement("w:pPr")
        carrier.insert(0, pPr)
    for old in pPr.findall(qn("w:sectPr")):
        pPr.remove(old)
    
    # Add continuous section break type
    sectPr = OxmlElement("w:sectPr")
    type_el = OxmlElement("w:type")
    type_el.set(qn("w:val"), "continuous")  # ← KEY: No page break!
    sectPr.append(type_el)
    
    # Copy margin settings from letter_sectPr
    for child in letter_sectPr:
        if child.tag != qn("w:type"):  # Avoid duplicate type
...
def append_tables_to_docx(docx_path: Path, xlsx_data: dict, output_path: Path,
    ...
    def numeric_font_size(text, base=7):
        digits = len(str(text))
        if digits >= 16:
            return max(6, base - 1)
        if digits >= 14:
            return max(6.5, base - 0.5)
        return base
    ...
    def set_row_cant_split(row):
        """Tell Word not to split this row across pages."""
    ...
    def set_row_repeat_header(row):
        """Repeat this row when the table continues on the next page."""
        trPr = row._tr.get_or_add_trPr()
    ...
    def set_row_keep_with_next(row):
        """
        Keep this row visually attached to the next row by setting keepNext
        and keepLines on every paragraph in the row's cells.
        """
    ...
    # Split letter pages and table page into separate Word sections so each
    # can carry independent margin settings.    
    _split_sections_before_table(doc)
    ...
    for ri, row_d in enumerate(rows_data):
        ...
        set_cell_text(cells[3], row_d["due_date"], size=7,
                      align=WD_ALIGN_PARAGRAPH.CENTER)
        ostd_txt = fmt_num(row_d["ostd"])
        dpp_txt = fmt_num(row_d["dpp"])
        set_cell_text(cells[4], ostd_txt, size=numeric_font_size(ostd_txt, 7),
                      align=WD_ALIGN_PARAGRAPH.RIGHT, no_wrap=True)
        set_cell_text(cells[5], dpp_txt, size=numeric_font_size(dpp_txt, 7),
                      align=WD_ALIGN_PARAGRAPH.RIGHT, no_wrap=True)
        for di, val in enumerate(row_d["rewards"]):
            reward_txt = fmt_num(val)
            set_cell_text(cells[base_cols + di], reward_txt,
                          size=numeric_font_size(reward_txt, 7),
                          align=WD_ALIGN_PARAGRAPH.RIGHT, no_wrap=True)
        for c in cells:
            set_cell_margins(c)
    ...
    total_ostd_txt = fmt_num(totals.get("ostd"))
    total_dpp_txt = fmt_num(totals.get("dpp"))
    set_cell_text(tot.cells[4], total_ostd_txt, bold=True,
                  size=numeric_font_size(total_ostd_txt, 8),
                  align=WD_ALIGN_PARAGRAPH.RIGHT, no_wrap=True)
    set_cell_text(tot.cells[5], total_dpp_txt, bold=True,
                  size=numeric_font_size(total_dpp_txt, 8),
                  align=WD_ALIGN_PARAGRAPH.RIGHT, no_wrap=True)
    reward_totals = totals.get("rewards", [])
    for di, val in enumerate(reward_totals):
        reward_total_txt = fmt_num(val)
        set_cell_text(tot.cells[base_cols + di], reward_total_txt,
                      bold=True, size=numeric_font_size(reward_total_txt, 8),
                      align=WD_ALIGN_PARAGRAPH.RIGHT, no_wrap=True)
    ...
    for si, (label, vals) in enumerate(summaries):
        ...
        for di in range(n_date_cols):
            ...
            summary_txt = fmt_num(val)
            set_cell_text(rr.cells[base_cols + di], summary_txt,
                          bold=True, size=numeric_font_size(summary_txt, 8),
                          align=WD_ALIGN_PARAGRAPH.RIGHT, no_wrap=True)
    ...
    # Pagination controls:
    # - Rows should not split across pages.
    # - Header rows should repeat when the table continues.
    # - The final Total/PPH/Net/Nominal block should move as a unit.
    for row in table.rows:
        set_row_cant_split(row)
    set_row_repeat_header(table.rows[0])
    set_row_repeat_header(table.rows[1])
    for row_idx in range(tot_row_idx, tot_row_idx + 3):
        set_row_keep_with_next(table.rows[row_idx])

    set_table_borders(table)

    # ── Keep Total + summary rows together — never split across pages ──
    def _cant_split(row):
        from docx.oxml import OxmlElement
        from docx.oxml.ns import qn
        trPr = row._tr.get_or_add_trPr()
    ...
    _cant_split(table.rows[tot_row_idx])
    for si in range(len(summaries)):
        _cant_split(table.rows[tot_row_idx + 1 + si])      

    # Column widths in cm. Numeric columns are intentionally wider and marked
    # no-wrap so billion-scale rupiah values stay on one line.
    #   Payer Name   1.6 cm    names may wrap; this is preferable to wrapping money
    ...
    #   Ostd.        2.4 cm    "10,000,000,000" and similar totals
    #   DPP          2.4 cm    same
    # Total base = 10.8 cm, leaving ~8.2 cm for the date columns (typically 3-4).
    base_widths_cm = [1.6, 1.4, 1.5, 1.5, 2.4, 2.4]
    ...
    # Explicitly add a near-invisible trailing paragraph after the table.
    # Word requires the body to end with <w:p>; without this, Word COM
    # inserts a full-height one that can spill to a blank final page
    # when the table nearly fills the last page.
    _trail = doc.add_paragraph()
    _trail.paragraph_format.space_before = Pt(0)
    _trail.paragraph_format.space_after  = Pt(0)
    _trail.add_run().font.size = Pt(1)
...
def convert_to_pdf(docx_path: Path, pdf_path: Path) -> bool:
    """
    Convert DOCX → PDF using Microsoft Word COM.
    Requires: pip install comtypes
    """
    if sys.platform != "win32":
        _print("  [!] Word COM only available on Windows", "error")
        return False
    ...
    try:
        ...
        _print("  [INFO] Starting Microsoft Word for PDF conversion...", "info")
        ...
        # DocStructureTags=0 → wdExportDocumentContent
        doc.ExportAsFixedFormat(
            OutputFileName=str(pdf_path.resolve()),
            ExportFormat=17,        # ✅ FIX: Changed from FileFormat to ExportFormat
        ...
        if pdf_path.exists():
            _print(f"  [OK] PDF created via Word COM: {pdf_path.name}", "success")
            return True
        return False
            
    except ImportError:
        _print("  [!] comtypes not installed. Run: pip install comtypes", "error")
        return False
    except Exception as e:
        _print(f"  [!] Word COM conversion error: {e}", "error")
        return False
    finally:
